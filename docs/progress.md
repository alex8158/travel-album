...
继续 **P9.T4 [SHOULD]**：`video_proxy` 720p 低清代理 worker —— FFmpeg 转码原视频到 H.264 720p（CRF 28），写到 `derived/{mediaId}/proxy.mp4`，更新 `media_items.preview_path` 指向代理（与 P9.T3 写 `thumbnail_path` 平行）。这能让前端视频详情页用代理而非全分辨率原视频做内联播放。
