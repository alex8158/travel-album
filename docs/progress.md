...
继续 **P9.T3 [MUST]**：`video_cover` worker —— 使用 FFmpeg 抽取一帧作为视频封面（写到 `derived/{mediaId}/cover.jpg`、更新 `media_items.thumbnail_path` / `preview_path` 或写 `media_versions(version_type='video_cover')`）。P9.T2 已经把 ffprobe metadata 准备好，封面帧的抽取时点（建议 1s 处或 duration/2）可以从 P9.T2 写入的 duration 决定。
