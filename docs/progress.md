...
继续 **P9.T8 [SHOULD]**：Video API（design.md §9.5 + requirements.md §10.7）—— 暴露 segments / process endpoints 给前端读取 P9.T1 ~ P9.T7 的产物：每段的 startTime / endTime / duration / blur_score / quality_score / waste_type / is_recommended / user_decision / reason，以及 keyframes / cover / proxy 的派生文件路径。任务范围**只**包含读 + 用户手动 user_decision 写入（PATCH segment user_decision），不引入智能剪辑或 AI 视频处理。注意：P9.T8 落地的 user_decision PATCH 端点要复用 `videoSegmentsRepo.updateUserDecision(id, userDecision, now)`（P9.T7 已经公开）；如果后续要触发"重跑 + force"，需要在 enqueue path 上加 payload `{"force":true}` 显式标记。
