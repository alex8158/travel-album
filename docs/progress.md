...
继续 **P10.T7 [MUST]**：阶段 P10 验收（requirements §7.10 验收 5 条 + "未配置 AI 时基础功能不受影响"）—— 跑端到端：上传一张图 → 默认 NoopProvider 下 AI Refine 按钮置灰 + 服务器 501 一致；切换到 `AI_ENABLED=true` + 自定义 LocalMockProvider（available=true + invoke 返回原图副本）→ AI Refine 按钮启用 → 二次确认 → POST → audit pending → worker drain → ai_invocations.success + media_versions(version_type='ai_refined') + UI 第三个 VersionCell 出现。同时压测 quota（daily / per-trip）和 P10.T5 R-132（retry 路径设 maxRetries=0 验证）。注意：P10.T7 不写真实 provider；可在 server 侧加一个 `LocalMockProvider`（仅 dev / test config 注册）作为本地验收 fixture。验收清单覆盖 R-121 ~ R-137；通过则 P10 整体收口。
