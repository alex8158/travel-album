...
继续 **P9.T7 [SHOULD]**：片段质量评分（模糊评分 on 关键帧 + FFmpeg `blackdetect`）—— 把 P9.T5 落下的 `derived/{mediaId}/frames/manifest.json` 当作发现层，对每帧跑 sharp Laplacian 模糊分；同时 FFmpeg `blackdetect` 跑在 P9.T4 proxy 或 originalPath 上，按时间区间映射到 P9.T6 落下的 `video_segments` 行，UPDATE 每行的 `blur_score` / `stability_score` / `quality_score` / `waste_type` / `is_recommended`。**特别注意 R-107**：P9.T7 必须先决定如何在重跑时保留用户的 `user_decision`，不能直接被 P9.T6 的 wipe + reinsert 清掉。
