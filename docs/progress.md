...
继续 **P8.T5 [MUST]**：前端 —— 原图 vs 增强图对比、采用 / 放弃、重新增强。后端契约已完整（P8.T1 入队 → P8.T2/T3 sharp 处理 + 写 media_versions → P8.T4 列出可选版本 + 切换 active），现在 P8.T5 负责在 media 详情页串联这些端点，提供 before/after 对比 + "采用 / 放弃 / 重新增强" 三态按钮。
