...
继续 **P7.T3 [MUST]**：重复组批量删除 —— `POST /api/duplicate-groups/:id/delete-others`，走软删除路径（即 `MediaService.softDeleteMedia` 已有的事务路径），事务内对 group 内 `recommendation = 'remove'` / `user_decision = 'remove'` 的成员逐个软删；保留 group 推荐图本身；权限不去检查 user_confirmed = 1（用户已经决定），但内部仍由 softDeleteMedia 的兜底事务清理引用。
