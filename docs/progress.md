...
继续 **P10.T1**（按 tasks.md P10 子任务顺序）。P10 起步通常是 schema + AIProvider 接口（design.md §2.3 / §6.2），保持 AI 默认关闭、未配置 AI 时基础功能必须仍可用（CLAUDE.md §2.8）。P9 已经把 video 通道的 worker / API / UI 全部稳定下来，P10 走 ai 通道，互不干扰；同时复用 P9.T7 建立的 R-107 / time-overlap 保留机制思路也可指导未来 image_ai_refine 重跑用户决策的处理。
