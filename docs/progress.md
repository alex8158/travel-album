...
继续 **P8.T4 [MUST]**：版本切换 API（`GET /api/media/:id/versions` + `POST /api/media/:id/select-version`）—— 让用户能选择"采用 enhanced 版本"或"回到原图"。P8.T3 已经落地，所以 `media_versions(version_type='enhanced')` 行就绪可被这两个端点读取。
