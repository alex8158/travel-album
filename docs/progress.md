...
继续 **P8.T2 [MUST]**：sharp 增强管线 —— 在 `imageEnhanceWorker.ts` 现有 `IMAGE_ENHANCE_JOB_TYPE` 常量旁追加 `makeImageEnhanceHandler` + deps，使用 sharp 完成白平衡 / 曝光 / 对比度 / 锐化 / 降噪（参数 from config）。注意 P8.T1 已经让队列可以接收任务，但目前没有 handler，job 会停留在 pending —— P8.T2 落地后即可消化。
