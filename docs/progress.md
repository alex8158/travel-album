...
继续 **P9.T5 [SHOULD]**：`video_keyframes` worker —— 在固定间隔（默认 `VIDEO_KEYFRAME_INTERVAL=2s`）从代理或原视频抽帧到 `derived/{mediaId}/frames/`。这是为 P9.T6 segment quality + P9.T7 segment 模糊评分准备帧素材的基础步骤。注意 P9.T4 写了 `video_proxy.mp4`，理论上 keyframes 可以从代理而非原视频抽（更快）；P9.T5 prompt 落地时再定。
