...
继续 **P9.T10 [MUST]**：阶段 P9 验收（requirements.md §7.11 + §7.12 已实现部分）—— 跑端到端验收：上传一段 12s+ 视频 → 等 P9.T2/T3/T4/T5/T6/T7 全部 success → 在 P9.T9 的视频片段页确认 segments 列表、quality 评分、keyframe 缩略、user_decision 三键、re-analyse / force re-analyse 都按设计工作。同时检查 §7.18 软删除契约：软删除视频后访问其 video segments 页应返 404，restore 后再访问应恢复。注意：P9.T10 是验收 + 文档对齐任务，不再写新 worker / API / 页面；如发现 P9.T1-T9 有验收门没过的，补 minimal fix 而不是大改。
