...
继续 **P9.T6 [SHOULD]**：`video_segments` 固定时长切片（默认 10s 可配置）—— FFmpeg `-f segment -segment_time 10` 把视频按固定时长切成 N 个 `.mp4` 段，写到 `derived/{mediaId}/segments/{segmentId}.mp4`，并往 `video_segments` 表（P9.T1 已落库）INSERT 一行 per 段（包括 start_time / end_time / duration / 默认 waste_type='none' / is_recommended=0）。P9.T5 keyframes 的 manifest.json 可作为后续 P9.T7 段质量评分的输入。
