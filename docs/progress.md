...
继续 **P9.T9 [SHOULD]**：前端视频片段页（requirements.md §10.7 + design.md §10）—— 消费 P9.T8 落下的 4 个 endpoint：(1) GET /api/media/:mediaId/video-segments 渲染 segments 列表 + per-segment 缩略 + blur_score / quality_score / waste_type 图标；(2) GET /api/media/:mediaId/video-segments/:segmentId 用于详情展开 / 试播；(3) PATCH /api/video-segments/:segmentId/user-decision 接 "保留 / 删除" 按钮；(4) POST /api/media/:mediaId/process-video-segments 接 "重新分析 / 强制重置"按钮（force=true 由 UI 显式确认对话框触发）。P9.T8 已经把 keyframes manifest 摘要内联在 list 响应里，前端可以直接渲染轨道预览。注意：P7 软删除契约下前端不应让用户点开已删除媒体的视频片段页（API 会 404，前端早 disable 入口）。
