...
| R-72 | 没有"批量恢复" / "撤销 delete-others" 入口。用户对单个媒体可以走 P7.T2 restore，但一次批删 N 张要恢复 N 次。回收站 UI（P7.T4）会提供更人性化路径 | P7.T4 |
...
继续 **P7.T4 [MUST]**：前端回收站视图 —— 列出 `deleted_at` 不为空的媒体（需服务端补一个 `?includeDeleted=true|only` 查询能力或单独的 `GET /api/trips/:tripId/recycle-bin`），加恢复按钮（复用 P7.T2 `restoreMedia` API），软删除二次确认提示"可恢复"（部分已在 P7.T1 / T3 的 modal 中实现）。
